...
using NewtonVR;
public class NVRTeleporting : MonoBehaviour {
		...
		//-------------------------------------------------
		private static NVRTeleporting _instance;
		public static NVRTeleporting instance
		{
			get
			{
				if ( _instance == null )
				{
					_instance = GameObject.FindObjectOfType<NVRTeleporting>();
				}
		...
		private void InitiateTeleportFade()
		{
			...
			//NVRReplace//headAudioSource.transform.SetParent( player.hmdTransform );
			headAudioSource.transform.SetParent( player.transform );
		...
		private IEnumerator TeleportHintCoroutine()
		{
			...
			while ( true )
			{
				...
				foreach ( NVRHand hand in player.Hands )
				{
					...
					//NVRReplace//bool isShowingHint = !string.IsNullOrEmpty( ControllerButtonHints.GetActiveHintText( hand, EVRButtonId.k_EButton_SteamVR_Touchpad ) );
					...
					if ( showHint )
					{
						...
						if ( Time.time > prevHapticPulseTime + 0.05f )
						{
							...
							//NVRReplace//hand.controller.TriggerHapticPulse( 500 );
		...
		//-------------------------------------------------
		private bool WasTeleportButtonReleased( NVRHand hand )
		{
			if ( IsEligibleForTeleport( hand ) )
			{
				...
				return hand.Inputs[NVRButtons.Touchpad].PressUp;
			}
		...
		//-------------------------------------------------
		private bool IsTeleportButtonDown( NVRHand hand )
		{
			if ( IsEligibleForTeleport( hand ) )
			{
				//NVRReplace
				...
				return hand.Inputs[NVRButtons.Touchpad].IsPressed;
		...
		//-------------------------------------------------
		private bool WasTeleportButtonPressed( NVRHand hand )
		{
			if ( IsEligibleForTeleport( hand ) )
			{
				//NVRReplace
				...
				return hand.Inputs[NVRButtons.Touchpad].PressDown;
			}
		...
		//-------------------------------------------------
		private Transform GetPointerStartTransform( NVRHand hand )
		{
			//NVRReplace
			/* if ( hand.noSteamVRFallbackCamera != null )
			{
